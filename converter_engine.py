...
class QCVWebEngine:
    ...
    def _parse_cv_to_json(self, source_text: str, *, qcv_generated: bool = False) -> dict:
        master_prompt = self.config["prompt_master_inst"]
        schema = getattr(core, "CV_JSON_SCHEMA", "{}")
        qcv_hint = ""
        if qcv_generated:
            qcv_hint = (
                "\n\n**CRITICAL — RE-IMPORT MODE: This document was generated by our own CV system. "
                "It is already perfectly structured. Your ONLY job is to extract data verbatim.\n"
                "- Copy every Environment/skills list CHARACTER-FOR-CHARACTER. Do NOT add, remove, or reorder any items.\n"
                "- Do NOT infer technologies from project descriptions.\n"
                "- Do NOT fix dates or reorder anything.\n"
                "- If a field is empty in the source, it MUST be empty in the output.**\n"
            )
        full_prompt = (
            f"{master_prompt}{qcv_hint}\n\n"
            f"JSON SCHEMA:\n{schema}\n\n"
            f"SOURCE CV TEXT:\n{source_text}"
        )
    ...
    def _parse_cv_file_to_json(self, source_path: Path | str) -> tuple[dict, bool]:
        """Parse a CV file and return (data, qcv_generated) tuple."""
        source_path = Path(source_path)
        ...
        # Check for qcv_generated property in DOCX files
        qcv_generated = False
        if source_path.suffix.lower() == ".docx":
            try:
                from source_baseline_extractor import _check_qcv_property
                qcv_generated = _check_qcv_property(source_path)
            except Exception:
                pass

        source_text = read_source_text(source_path)
        return self._parse_cv_to_json(source_text, qcv_generated=qcv_generated), qcv_generated
    ...
    def process(
        ...
        *,
        ...
        if preloaded_data is not None:
            ...
            data, qcv_generated = self._parse_cv_file_to_json(source_path)
            ...
            self._debug(
                ...
                + (f"\nqcv_generated=true" if qcv_generated else ""),
            )
        # Store base CV JSON for download
        self._last_base_json = copy.deepcopy(data)

        if autofix and not qcv_generated:
        ...
        if tailor and jd_text.strip():
            ...
            try:
                ...
                if pause_event is not None:
                    ...
                    # 4h backstop so orphaned threads eventually die (server restart will too).
                    # Gap analysis is already persisted to store; exiting here = valid "gap-only" outcome.
                    if not pause_event.wait(timeout=14400):
                        raise RuntimeError("Gap analysis timed out — user did not proceed.")
